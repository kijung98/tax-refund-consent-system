...
/**
 * 접수 목록(submissions + form_b_joint_home / form_a_transfer)을 엑셀 파일로 다운로드
 * FORM A, FORM B가 섞여 있어도 한 시트에서 고정된 컬럼셋으로 출력한다.
 * (SheetJS는 기본적으로 배열의 첫 행 기준으로 컬럼을 잡기 때문에,
 *  서식마다 다른 키를 그대로 넘기면 일부 컬럼이 누락될 수 있어
...
function exportSubmissionsToExcel(rows, filename) {
  ...
  const excelRows = rows.map(row => {
    ...
    const formType = b ? '공동명의 1주택자 특례신청서' : (a ? '국세환급금양도요구서' : '-');
